            ...
            <div class="col">
				...
				<div id="id-city" class="city-container"<?= $this->user->country ? ($this->user->country=='ID' ? '' : ' style="display:none"') : '' ?>>
					...
					<div class="custom-control custom-switch mt-5 mr-3">
						<input type="checkbox" class="custom-control-input" id="rajaongkir_enabled" name="rajaongkir_enabled" value="<?= $data->configs['enabled']&&$this->user->ro_pro_package ?>"<?= $this->user->ro_pro_package ? ' checked' : '' ?>>
					...
					<div class="rajaongkir-container<?= !$data->configs['enabled'] ? ' ro-disabled' : '' ?>">
						...
						<?php $is_eshop_pro=false; if($data->ro_package && $data->url_biolink){?>
						<div class="form-group">
						<label><b>EShop Pro Biolink</b></label>
						<div id="accordion">
						<?php foreach($data->ro_biolink as $rkey => $rval){
						if($data->ro_expired>strtotime('NOW') || $data->ro_expired[$rkey]=='lifetime'){
						$is_eshop_pro=true;
						?>
							<div class="card">
								<div class="card-header" data-toggle="collapse" data-target="#collapse<?= $rkey ?>" aria-expanded="true" aria-controls="collapse<?= $rkey ?>" id="heading<?= $rkey ?>">
									<div class="d-flex justify-content-between align-items-center">
										...
										<div>
											<?php if($data->ro_expired[$rkey]!='lifetime'):?>
											<p class="text-center mb-0">Expired</p>
											<p class="text-center text-primary mb-0"><?= $data->ro_expired[$rkey]!='lifetime' ? date('j F Y H:i A',$data->ro_expired[$rkey]) : 'Lifetime' ?></p>
											<?php endif;?>
										</div>
									</div>
								</div>
						...
						<?php }?>
						<?php if(!$is_eshop_pro){?>
							Jika anda ingin order lagi eshop pro silahkan klik tombol dibawah ini
							<br><br>
						...
						<div class="alert alert-success">
							<div class="text-center">Jika anda ingin order eshop pro silahkan klik tombol dibawah ini</div>
							<br>
							<div class="d-flex justify-content-center">
								<a href="https://wa.me/6289521009988?text=Halo%20Kang%20Ridwan%2C%20Saya%20Mau%20Order%20Layanan%20EShop%20Pro" class="btn btn-primary" target="_blank">Order EShop Pro</a>
							</div>
						...
						<div class="form-ro-packages">
							<?php if($this->user->ro_pro_package==0){?>
							<div class="form-group mb-0">
								<label for="name"><?= $data->configs['package']==2 ? 'Kurir EShop versi Pro' : 'Kurir EShop versi Free' ?></label>
							</div>
