...
<div class="d-flex justify-content-between">
    <div class="d-flex align-items-center">
        <h1 class="h3 mr-3"><i class="fa fa-fw fa-xs fa-box-open text-gray-700"></i> <?= sprintf($this->language->admin_package_update->header, $data->package->name) ?></h1>
        ...
        <form action="" method="post" role="form">
            ...
            <div class="row">
                ...
                <?php else: ?>
					...
					<div class="col-6">
						<div class="form-group form-nwoption"<?= $data->package->is_trial == '1' ? ' style=""' : ' style="display:none"' ?>>
							<label><?= $this->language->admin_packages->input->trial_expired ?></label>
							<select class="form-control" name="trial_expired">
								<option value="1"<?= $data->package->trial_expired == '3 days' ? ' selected' : null ?>>3 Days</option>
								<option value="2"<?= $data->package->trial_expired == '1 week' ? ' selected' : null ?>>1 Week</option>
								<option value="3"<?= $data->package->trial_expired == '2 weeks' ? ' selected' : null ?>>2 Weeks</option>
								<option value="4"<?= $data->package->trial_expired == '3 weeks' ? ' selected' : null ?>>3 Weeks</option>
								<option value="5"<?= $data->package->trial_expired == '1 month' ? ' selected' : null ?>>1 Month</option>
								<option value="6"<?= $data->package->trial_expired == '3 month' ? ' selected' : null ?>>3 Month</option>
							</select>
						</div>
					</div>
					...
					<div class="col-6">
						<div class="form-group form-nwoption"<?= $data->package->is_trial == '1' ? ' style=""' : ' style="display:none"' ?>>
							<label><?= $this->language->admin_packages->input->is_default ?></label>
							<select class="form-control" name="is_default">
								<option value="1"<?= $data->package->is_default == '1' ? ' selected' : null ?>><?= $this->language->global->yes ?></option>
								<option value="0"<?= $data->package->is_default == '0' ? ' selected' : null ?>><?= $this->language->global->no ?></option>
							</select>
						</div>
					</div>
            ...
            <div class="form-group" <?= !$this->settings->links->domains_is_enabled ? 'style="display: none"' : null ?>>
                ...
                <input type="number" id="domains_limit" name="domains_limit" min="-1" class="form-control" value="<?= $data->package->settings->domains_limit ?>" />
			...
			<div class="custom-control custom-switch mb-3">
                <input id="exclude_agency" name="exclude_agency" type="checkbox" class="custom-control-input" <?= isset($data->package->settings->exclude_agency) ? ($data->package->settings->exclude_agency ? 'checked="true"' : '') : '' ?>>
                <label class="custom-control-label" for="exclude_agency"><?= $this->language->admin_packages->input->exclude_agency ?></label>
                <div><small class="text-muted"><?= $this->language->admin_packages->input->exclude_agency_help ?></small></div>
            </div>
			<?php }?>
    ...
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <form action="" method="post" role="form">
                ...
                <button type="submit" name="submit" class="btn btn-secondary"><?= $this->language->global->update ?></button>
            </form>
        </div>
    </div>
</div>
<script>
$('[name="is_trial"]').on('change',function(e){
	if($(this).val()==1)
		$('.form-nwoption').show();
	else
		$('.form-nwoption').hide();
