...
<html class="admin" lang="<?= $this->language->language_code ?>">
    <head>
        ...
        <?php foreach([\Altum\ThemeStyle::get_file(), 'custom.css', 'admin-custom.css', 'animate.min.css'] as $file): ?>
            <link href="<?= SITE_URL . ASSETS_URL_PATH ?>css/<?= $file ?>?v=<?= PRODUCT_CODE ?>" rel="stylesheet" media="screen">
        <?php endforeach ?>
			<link href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-select/1.13.18/css/bootstrap-select.min.css" rel="stylesheet" media="screen">
    ...
    <?php endforeach ?>
	<script src="<?= SITE_URL . ASSETS_URL_PATH ?>js/libraries/clipboard.min.js?v=<?= PRODUCT_CODE ?>"></script>
