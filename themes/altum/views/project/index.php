...
<section class="container">
    ...
    <div class="margin-top-3 d-flex justify-content-between">
        <h2 class="h4"><?= $this->language->project->links->header ?>
            <!--
            <br><br>
            <a type="button" class="btn btn-primary rounded-pill" target="_blank" href="https://<?= BASE_DOMAIN ?>/wajib">
                PENTING!!! WAJIB KLIK DISINI
            </a><br><br>
            -->
        </h2>
        
        <div class="col-auto p-0">
            <button type="button" data-toggle="modal" data-target="#project_export" class="btn btn-primary rounded-pill">
                Export Project
            </button>
        </div>
                ...
                <div class="dropdown">
                    <a href="#" data-toggle="dropdown" class="text-secondary dropdown-toggle dropdown-toggle-simple">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            ...
                            <a href="#" data-toggle="modal" data-target="#duplicate_biolink" data-biolink-id="<?= $row->link_id?>" data-name="<?= $row->url?>" class="dropdown-item duplicate-biolink"><i class="fa fa-fw fa-copy"></i> <?= $this->language->global->duplicate ?></a>
...
<script>
    ...
    new Chart(clicks_chart, {
        type: 'line',
        data: {
            labels: <?= $data->logs_chart['labels'] ? $data->logs_chart['labels'] : 'Chart' ?>,
            datasets: [{
...
<script>
$('#type_duplicate').off('change').on('click',function(e) {
	if($(this).val()==0)
		$('#ptype_duplicate').show();
	else
		$('#ptype_duplicate').hide();
})
</script>
