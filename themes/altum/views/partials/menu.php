...
<nav class="
    navbar
    navbar-main
    <?= \Altum\Routing\Router::$controller_settings['menu_no_margin'] ? null : 'margin-bottom-6'?>
    navbar-expand-lg
    <?= \Altum\Routing\Router::$controller_key == 'index' ? 'navbar-absolute' : null ?>
    <?= \Altum\Routing\Router::$controller_key == 'index' && \Altum\ThemeStyle::get() == 'light' ? 'navbar-dark navbar-index' : 'navbar-light ' ?>
">
    <div class="container">
        <a class="navbar-brand" href="<?= url() ?>">
			<?php if($this->user->whitelabel == 'Y' || $_SERVER['SERVER_NAME'] == strtolower(whitelabel('url'))): ?>
					...
					<li class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            <?php if(\Altum\Middlewares\Authentication::is_admin()): ?>
								...
								<?php elseif($this->user->superagency=='Y'): ?>
                                <a class="dropdown-item" href="<?= url('superagency') ?>"><i class="fa fa-fw fa-sm fa-user-shield mr-1"></i> <?= $this->language->global->menu->superagency ?></a>
								<?php elseif($this->user->agency=='Y'): ?>
                                <a class="dropdown-item" href="<?= url('agency') ?>"><i class="fa fa-fw fa-sm fa-user-shield mr-1"></i> <?= $this->language->global->menu->agency ?></a>
								<?php elseif($this->user->subagency=='Y'): ?>
								<a class="dropdown-item" href="<?= url('subagency') ?>"><i class="fa fa-fw fa-sm fa-user-shield mr-1"></i> <?= $this->language->global->menu->subagency ?></a>
								<?php else: ?>
