    ...
    public static function get_biolink($tthis, $link, $user = null, $links = null) {
        ...
        switch($link->settings->background_type) {
            case 'image':
                ...
                $link->design->background_style = 'background: url(\'' . SITE_URL . UPLOADS_URL_PATH . 'backgrounds/' . $link->settings->background . '\');background-size:100% auto !important;background-repeat:repeat-y !important;background-position: top;';

                break;

            case 'gradient':
				
				if(isset($link->settings->background->type)&&$link->settings->background->type=='linear')
					$link->design->background_style = 'background-image: linear-gradient('.(isset($link->settings->background->degree)&&$link->settings->background->degree ? $link->settings->background->degree : 135).'deg, ' . $link->settings->background->color_one . ' 10%, ' . $link->settings->background->color_two . ' 100%);';
				elseif(isset($link->settings->background->type)&&$link->settings->background->type=='radial')
					$link->design->background_style = 'background-image: radial-gradient(circle, ' . $link->settings->background->color_one . ' '.(isset($link->settings->background->degree)&&$link->settings->background->degree ? $link->settings->background->degree : 135).'%, ' . $link->settings->background->color_two . ' 100%);';
				else
					$link->design->background_style = 'background-image: linear-gradient('.(isset($link->settings->background->degree)&&$link->settings->background->degree ? $link->settings->background->degree : 135).'deg, ' . $link->settings->background->color_one . ' 10%, ' . $link->settings->background->color_two . ' 100%);';
			...
			case 'anigradient':
				...
				$link->design->background_style = 'background: linear-gradient('.(isset($link->settings->background->degree)&&$link->settings->background->degree ? $link->settings->background->degree : 110).'deg, ' . $link->settings->background->color_one . ', ' . $link->settings->background->color_two . ', ' . $link->settings->background->color_three . ', ' . $link->settings->background->color_four . ');';
		...
		if($user->country=="MY") {
			...
			$subdistrict = Database::$database->query("SELECT * FROM `kecamatan` WHERE kc_kt_id = 151");
    ...
    public static function get_biolink_link($link, $user = null) {
        ...
        switch($link->subtype) {
            ...
            case 'youtube':
                ...
                if(preg_match("/^(?:http(?:s)?:\/\/)?(?:www\.)?(?:m\.)?(?:youtu\.be\/|youtube\.com\/(?:(?:watch)?\?(?:.*&)?v(?:i)?=|(?:embed|v|vi|user|shorts)\/))([^\?&\"'>]+)/", $link->location_url, $match)) {
                    $data['embed'] = $match[1];
