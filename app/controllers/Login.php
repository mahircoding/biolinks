...
class Login extends Controller {
    ...
    public function index() {
		...
		if(trim($_SERVER['SERVER_NAME'])!=BASE_DOMAIN) {
			...
			$dom_wl_exists = Database::get('*', 'whitelabel', ['url' => $main_server_name]);
			//$dom_dom_exists = Database::get('*', 'domains', ['host' => $main_server_name]);
			//var_dump($dom_dom_exists);exit;
			
			if(!$dom_wl_exists) {
				//if(!$dom_wl_exists && !$dom_dom_exists) {
                ...
                if(isset($response)) {
                    ...
                    /* Create a new account */
                        ...
                        if(empty($_SESSION['error'])) {
                            ...
                            $total_logins               = 1;
                            $package_id                 = 'free';
                            $package_expiration_date    = \Altum\Date::$date;
        ...
        if(!empty($_POST)) {
			...
			if(trim($_SERVER['SERVER_NAME'])==BASE_DOMAIN) {
				...
				$result = Database::$database->query("SELECT a.`user_id`, a.`email`, a.`active`, a.`password`, a.`token_code`, a.`total_logins`, a.`twofa_secret`, b.`url` FROM `users` a LEFT JOIN `whitelabel` b ON b.`id` = a.`whitelabel_id` WHERE a.`email` = '{$_POST['email']}' AND b.url = '{$main_server_name}' AND b.url IS NOT NULL");
				$login_account = $result->num_rows ? $result->fetch_object() : false;
			}
        ...
        $data = [
			...
			'is_main_url' => $main_server_name ? false : true,
        ];
