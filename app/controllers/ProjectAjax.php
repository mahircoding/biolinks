...
class ProjectAjax extends Controller {
    ...
    private function delete() {
        ...
        if(empty($errors)) {
			...
			$result = Database::$database->query("SELECT * FROM links WHERE project_id = {$_POST['project_id']} and user_id = {$this->user->user_id} and subtype = 'base'");
			while($links = $result->fetch_object()) {
				if($links->type == 'biolink' && $links->subtype == 'base') {
					if(is_dir(UPLOADS_PATH . 'galleries/' . $links->link_id)) {
						array_map('unlink', glob(UPLOADS_PATH . 'galleries/' . $links->link_id . "/*.*"));
						rmdir(UPLOADS_PATH . 'galleries/' . $links->link_id);
					}
				}
			}
