...
class Link extends Controller {
    ...
    public function index() {
        ...
        $original_url_host = strtolower(parse_url(url())['host']);
        $request_url_host = strtolower(Database::clean_string($_SERVER['HTTP_HOST']));
        
        $main_server_name = null;
		if(trim($_SERVER['SERVER_NAME']) != BASE_DOMAIN)
			$main_server_name = strtolower(trim($_SERVER['SERVER_NAME']));
        ...
        if($original_url_host == $request_url_host) {
            ...
            if($request_url_host==BASE_DOMAIN) {
               ...
               if($link_id) {
					...
					LEFT JOIN `whitelabel` ON `users`.`whitelabel_id` = `whitelabel`.`id`
					WHERE
						`links`.`url` = '{$link_url}' AND 
						`links`.`is_enabled` = 1 AND 
						(`domains`.`host` = '{$main_server_name}' OR `whitelabel`.`url` = '{$main_server_name}')
					LIMIT 1
					")->fetch_object() ?? null;
				}
            }
		...
		if($_POST) {
			...
			if(isset($_SESSION['password_protector'])) {
				$time_lock = isset($this->link->settings->timeout_lock) ? ($this->link->settings->timeout_lock=='custom' ? $this->link->settings->timeout_lock_custom * 60 : $this->link->settings->timeout_lock * 60) : 5 * 60;
        ...
        if(
			...
			if(
                strtotime((new \DateTime('now', new \DateTimeZone($this->user->timezone)))->format('Y-m-d H:i:s')) >= strtotime($this->link->start_date) &&
                strtotime((new \DateTime('now', new \DateTimeZone($this->user->timezone)))->format('Y-m-d H:i:s')) <= strtotime($this->link->end_date)
            ) {} 
			else {
				if($main_server_name)
					redirect('http://'.$main_server_name,true);
		...
		// To check if user expired trial, showing pop up expired 
        $this->expired_script = null;
		if(strtotime($this->user->package_expiration_date) < strtotime('NOW')) {
            $this->expired_script = '
            <script>$(document).ready(function() {setTimeout(function() {$("#expired").modal({backdrop: "static"});},1000);});</script><div id="expired" class="modal fade justify-content-center"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header"><h4 class="modal-title">Expired Account</h4></div><div class="modal-body"><p>Masa Coba Gratis Anda Sudah Habis. Biolink yang sudah anda buat kami nonaktifkan. Jika ingin tetap aktif silahkan hubungi orang yang sudah memperkenalkan aplikasi ini pada Anda lalu minta dia untuk mengupgrade akun Anda</p></div></div></div></div>';
        }
        ...
        if(!isset($_GET['preview'])) {
            ...
            if(!isset($_SERVER['HTTP_ACCEPT_LANGUAGE']))
				$_SERVER['HTTP_ACCEPT_LANGUAGE'] = 'id,en-US;q=0.7,en;q=0.3';
    ...
    public function process_biolink() {
		...
		foreach($links as $link) {
			$filter_contents .= $this->link->url . ' ' . $this->link->settings->title . ' ' . $this->link->settings->description . ' ' . $link->url . ' ' . $link->location_url . ' ' . $link->settings;
		}
		
		$is_blocked = is_link_contains_badwords($filter_contents);
