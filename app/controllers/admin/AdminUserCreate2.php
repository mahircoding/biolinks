...
class AdminUserCreate extends Controller {
    ...
    public function index() {
		...
		$result = Database::get('*', 'users', ['user_id' => $this->user->user_id]);
		$licenses_user = $result->ulicense;
		...
		if(!empty($_POST)) {
            ...
            if(empty($_SESSION['error'])) {
                ...
                $stmt = Database::$database->prepare("INSERT INTO `users` (`password`, `email`, `email_activation_code`, `name`, `phone`, `package_id`, `package_expiration_date`, `package_settings`, `active`, `date`, `ip`, `last_user_agent`, `total_logins`,`ids_insert`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param('ssssssssssssss', $password, $_POST['email'], $email_code, $_POST['name'], $_POST['phone'], $package_id, $package_expiration_date, $package_settings, $active, \Altum\Date::$date, $ip, $last_user_agent, $total_logins,$_POST['ids_insert']);
                $stmt->execute();
