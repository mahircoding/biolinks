...
class AdminSettings extends Controller {
    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            /* Filter Words Tab */
			$_POST['filter_filter_words'] = filter_var($_POST['filter_filter_words'], FILTER_SANITIZE_STRING);
            ...
            $_POST['whatsapp_notifications_whatsapps'] = trim($_POST['whatsapp_notifications_whatsapps']);
			$_POST['whatsapp_notifications_api_key'] = trim(str_replace(' ', '', $_POST['whatsapp_notifications_api_key']));
            $_POST['whatsapp_notifications_new_user'] = (bool) isset($_POST['whatsapp_notifications_new_user']);
            $_POST['whatsapp_notifications_new_payment'] = (bool) isset($_POST['whatsapp_notifications_new_payment']);
			
			/* Check Whatsapp notifications settings if exists */
			if(!$result = Database::get('*', 'settings', ['key' => 'whatsapp_notifications'])) {
				$stmt = Database::$database->prepare("INSERT INTO `settings` (`key`, `value`) VALUES ('whatsapp_notifications', '')");
                $stmt->execute();
                $stmt->close();
			}
            ...
            if(empty($_SESSION['error'])) {
                ...
                foreach ($settings_keys as $key => $value) {
                    ...
                    if(is_array($value)) {
                        ...
                        foreach($value as $sub_key) {
                            ...
                            if(!in_array($key . '_' . $sub_key, ['links_blacklisted_domains', 'links_blacklisted_keywords', 'custom_head_css', 'custom_head_js', 'ads_header', 'ads_footer', 'ads_header_biolink', 'ads_footer_biolink'])) {
                ...
                file_put_contents(UPLOADS_PATH . '/filters.db',$_POST['filter_filter_words']);
