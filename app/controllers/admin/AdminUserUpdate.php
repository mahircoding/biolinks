        ...
        if(!empty($_POST)) {
            ...
            if(empty($_SESSION['error'])) {
				...
				if($this->user->type == 1 && (empty($this->user->whitelabel) && empty($this->user->superagency) && empty($this->user->agency) && empty($this->user->subagency))) {
					...
					$stmt = Database::$database->prepare("UPDATE `users` SET `ro_pro_package` = ?  WHERE `user_id` = " . $user->user_id);
					$stmt->bind_param('s', $_POST['ro_status']);
					$stmt->execute();
					$stmt->close();
					
					/* Update 3 in 1 package if enabled */
					$esc_expired = $_POST['esc_expired']&&count(explode(' ',$_POST['esc_expired']))>1 ? $_POST['esc_expired'] : date('Y-m-d',strtotime($_POST['esc_expired'])).' '.date('H:i:s');
					if(!$_POST['esc_status']) $esc_expired = null;
					$stmt = Database::$database->prepare("UPDATE `users` SET `esc_package` = ?, `esc_expired` = ? WHERE `user_id` = " . $user->user_id);
					$stmt->bind_param('ss', $_POST['esc_status'], $esc_expired);
					$stmt->execute();
					$stmt->close();
