...
class LinkAjax extends Controller {
    ...
    private function duplicate() {
        ...
        /* Get the link data */
        $link = Database::get('*', 'links', ['link_id' => $_POST['link_id'], 'user_id' => $this->user->user_id, 'type' => 'biolink']);
        //$html = Database::get('*', 'links', ['link_id' => $_POST['link_id'], 'user_id' => $this->user->user_id, 'type' => 'biolink', 'subtype' => 'html']);
		if($link) {
			$link->settings = json_decode($link->settings);
			
			$duplicate_subtype = ['text','runningtext','waform','countdown','floatbutton','pricingtable','youtube','tiktok'];
			...
			if($link->subtype=='link') {
				...
				while(Database::exists('link_id', 'links', ['url' => $url, 'whitelabel_id' => $whitelabel_id])) {
					$url = string_generate(10);
				}

				$stmt = Database::$database->prepare("INSERT INTO `links` (`project_id`, `biolink_id`, `user_id`, `whitelabel_id`, `type`, `subtype`, `url`, `location_url`, `settings`, `start_date`, `end_date`, `date`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
				$stmt->bind_param('ssssssssssss', $link->project_id, $link->biolink_id, $this->user->user_id, $whitelabel_id, $link->type, $link->subtype, $url, $link->location_url, $settings, $link->start_date, $link->end_date, \Altum\Date::$date);
				$stmt->execute();
				$stmt->close();
			...
			} elseif($link->subtype=='html') {
				$order = 99;
				$url = string_generate(10);
				$settings = json_encode([
					'description' => $link->settings->description,
					'description_text_color' => $link->settings->description_text_color
				]);
				...
				$stmt->bind_param('ssssssssss', $link->project_id, $link->biolink_id, $this->user->user_id, $link->type, $link->subtype, $settings, $link->start_date, $link->end_date, \Altum\Date::$date, $order);
				$stmt->execute();
				$stmt->close();
			...
			} elseif(in_array($link->subtype,$duplicate_subtype)) {
				$url = $location_url = '';
				$order = 99;
    ...
    private function create_link() {
		...
		if(in_array($domain_id,[2172])) {
			if(!$this->user->esc_package) {
				Response::json('Silahkan anda membeli fitur paket 3 in 1, untuk memakai domain ini!.', 'error');
			}
		}
        ...
        if($_POST['url']) {
			if(Database::exists('link_id', 'links', ['url' => $_POST['url'], 'domain_id' => $domain_id, 'whitelabel_id' => $whitelabel_id])) {
        ...
        if(empty($errors)) {
            ...
            /* Generate random url if not specified */
			while(Database::exists('link_id', 'links', ['url' => $url, 'domain_id' => $domain_id, 'whitelabel_id' => $whitelabel_id])) {
				$url = string_generate(10);
			}
			
            /* Insert to database */
            $stmt = Database::$database->prepare("INSERT INTO `links` (`project_id`, `user_id`, `domain_id`, `whitelabel_id`, `type`, `subtype`, `url`, `location_url`, `settings`, `date`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param('ssssssssss', $_POST['project_id'], $this->user->user_id, $domain_id, $whitelabel_id, $type, $subtype, $url, $_POST['location_url'], $settings, \Altum\Date::$date);
            $stmt->execute();
    ...
    private function create_biolink() {
        ...
        /* Insert to database */
        $stmt = Database::$database->prepare("INSERT INTO `links` (`project_id`, `user_id`, `domain_id`, `whitelabel_id`, `type`, `subtype`, `url`, `settings`, `date`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param('sssssssss', $_POST['project_id'], $this->user->user_id, $domain_id, $whitelabel_id, $type, $subtype, $url,  $settings, \Altum\Date::$date);
        $stmt->execute();
        $link_id = $stmt->insert_id;
        $stmt->close();
	...
	private function create_duplicate_biolink() {
		$_POST['link_id'] = (int) $_POST['link_id'];
		$_POST['url'] = !empty($_POST['url']) ? get_slug(Database::clean_string($_POST['url'])) : false;
		$_POST['type_duplicate'] = (int) $_POST['type_duplicate'];
		...
		$project_id = $_POST['type_duplicate'];
		...
		/* Insert to database */
		$stmt = Database::$database->prepare("INSERT INTO `links` (`project_id`, `user_id`, `domain_id`, `whitelabel_id`, `type`, `subtype`, `url`, `settings`, `order`, `is_enabled`, `date`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		$stmt->bind_param('sssssssssss', $project_id, $this->user->user_id, $domain_id, $whitelabel_id, $type, $subtype, $url,  $settings,  $order,  $is_enabled, \Altum\Date::$date);
		$stmt->execute();
		...
		while($links = $result->fetch_object()) {
			...
			if($subtype=='link') {
				/* Insert to database */
				$stmt = Database::$database->prepare("INSERT INTO `links` (`project_id`, `user_id`, `biolink_id`, `domain_id`, `whitelabel_id`, `type`, `subtype`, `url`, `location_url`, `settings`, `order`, `is_enabled`, `date`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
				$stmt->bind_param('sssssssssssss', $project_id, $this->user->user_id, $biolink_id, $domain_id, $whitelabel_id, $type, $subtype, $url, $location_url,  $settings,  $order,  $is_enabled, \Altum\Date::$date);
				$stmt->execute();
				$stmt->close();
			} else {
				/* Insert to database */
    ...
    private function create_biolink_link() {
        ...
        $stmt = Database::$database->prepare("INSERT INTO `links` (`project_id`, `biolink_id`, `user_id`, `whitelabel_id`, `type`, `subtype`, `url`, `location_url`, `settings`, `order`, `date`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param('sssssssssss', $project_id, $_POST['link_id'], $this->user->user_id, $whitelabel_id, $type, $subtype, $url, $_POST['location_url'], $settings, $order, \Altum\Date::$date);
        $stmt->execute();
        $stmt->close();
    ...
    private function create_biolink_other($subtype) {
		...
		elseif($subtype=='tiktok') {
			$final_url = get_final_url_shortened($_POST['location_url']);
			$_POST['location_url'] = $final_url ? $final_url : $_POST['location_url'];
			$settings = null;
		} else
			$settings = null;
		$order = 99;
	...
	private function create_biolink_picture($subtype='picture') {
		...
		if($subtype=='picture') {
			...
			$settings = json_encode([
				...
				'slider_timer' => (float)$_POST['slider_timer']*1000,
			]);
		} else {
			$subtype = 'picture';
			$settings = json_encode([
	...
	private function create_biolink_eshop() {
		...
		if($_POST) {
			...
			for($img_num = 1; $img_num <= 4; $img_num++) {
				...
				if(isset($_FILES[$field_name]) && isset($_FILES[$field_name]['size'][$i][$j]) && $_FILES[$field_name]['size'][$i][$j] > 0) {
...
//dd("ok");
		...
		if(isset($_POST['domain_id'])) {
			if($r_domain = Database::get('*', 'domains', ['domain_id' => $_POST['domain_id']])) {
        ...
        if(!$link = Database::get('*', 'links', ['link_id' => $_POST['link_id'], 'user_id' => $this->user->user_id, 'whitelabel_id' => $whitelabel_id])) {
            die();
        }
...
//dd($_FILES['image']);
	...
	//dd("ok");
        ...
        switch($_POST['background_type']) {
			...
			case 'anigradient':
                ...
                $background = [
					...
					'degree' => (int)$_POST['background_animated_gradient_degree']
                ];
            ...
            case 'gradient':
                ...
                $background = [
                    'color_one' => $color_one,
                    'color_two' => $color_two,
					'type' => $_POST['background_gradient_type'],
					'degree' => $_POST['background_gradient_type']=='linear' ? (int)$_POST['background_gradient_degree'] : (int)$_POST['background_gradient_radius']
                ];
        ...
        //$_POST['seo_meta_image'] = Database::clean_string($_POST['seo_meta_image']);
		...
		$folder_id = $link->biolink_id ? $link->biolink_id : $link->link_id;
		
		if($_FILES['seo_meta_image']['size']>0) {
			if($link->settings->seo->meta_image&&file_exists(UPLOADS_PATH . 'galleries/' . $folder_id . '/' . $link->settings->seo->meta_image)) {
				unlink(UPLOADS_PATH . 'galleries/' . $folder_id . '/' . $link->settings->seo->meta_image);
			}
			
			require APP_PATH . 'includes/ResizeImage.php';
			$mime_type = getimagesize($_FILES['seo_meta_image']['tmp_name']);
			$img_ext = 'jpg';
			
			$resize = new \ResizeImage($_FILES['seo_meta_image']['tmp_name']);
			$resize->resizeTo(500, 500, 'maxWidth');
			...
			$meta_image = md5(time() . rand()) . '.' . $img_ext;
			
			/* Upload the image */
			$resize->saveImage(UPLOADS_PATH . 'galleries/' . $folder_id . '/' . $meta_image, '90', $img_ext);
		} else {
			$meta_image = isset($link->settings->seo->meta_image) ? $link->settings->seo->meta_image : '';
        ...
        $settings = json_encode([
			...
			'timeout_lock' => $_POST['password_protector'] ? $_POST['timeout_lock'] : 5,
			'timeout_lock_custom' => $_POST['password_protector'] ? $_POST['custom_timeout'] : 5,
    ...
    private function update_biolink_link() {
        ...
        if(isset($_POST['schedule']) && !empty($_POST['start_date']) && !empty($_POST['end_date']) && Date::validate($_POST['start_date'], 'Y-m-d H:i:s') && Date::validate($_POST['end_date'], 'Y-m-d H:i:s')) {
            $_POST['start_date'] = (new \DateTime($_POST['start_date'], new \DateTimeZone($this->user->timezone)))->format('Y-m-d H:i:s');
            $_POST['end_date'] = (new \DateTime($_POST['end_date'], new \DateTimeZone($this->user->timezone)))->format('Y-m-d H:i:s');
        } else {
		...
		if($_FILES['image']['size']>0) {
			...
			$picture_name = isset($images->image) ? $images->image : '';
		}
        ...
        if($_POST['url'] && $_POST['url'] != $link->url) {
			$rs_lk = Database::$database->query("SELECT a.`url` as l_url, b.`url` as w_url FROM `links` a LEFT JOIN `whitelabel` b ON a.`user_id` = b.`user_id` WHERE a.`url` = '{$_POST['url']}' AND b.`url` = '{$main_server_name}'");
			$lk_exists = $rs_lk&&$rs_lk->num_rows ? $rs_lk->num_rows : false;
			if($lk_exists)
				Response::json($this->language->create_biolink_modal->error_message->url_exists, 'error');
        } else if(!$_POST['url']) {
        ...
        $settings = json_encode([
            ...
            'icon' => $_POST['icon'],
			'image' => $picture_name ? SITE_URL . UPLOADS_URL_PATH . 'galleries/' . $folder_id . '/' . $picture_name : ''
        ]);
    ...
    private function update_biolink_other($subtype) {
        ...
        Response::json($this->language->link->success_message->settings_updated, 'success', ['final_url' => $_POST['location_url']]);
    }
    ...
    private function update_biolink_runningtext() {
        ...
        $settings = json_encode([
			...
			'background_color' => $_POST['background_color'],
			'background_radius' => $_POST['background_radius'],
					...
					if($_FILES['image']['size'][$i][$j]>0) {
						...
						if(isset($_POST['title_variant'][$i][$j])) {
							for($k=0;$k<count($_POST['title_variant'][$i][$j]);$k++) {
								...
								for($l=0;$l<count($_POST['name_variant'][$i][$j][$k]);$l++) {
									if(isset($_POST['name_variant'][$i][$j][$k][$l])) {
										$image_name = isset($images[$i]['products'][$j]['variants'][$k]['variant'][$l]['image_name']) ? $images[$i]['products'][$j]['variants'][$k]['variant'][$l]['image_name'] : null;
										$image_url = isset($images[$i]['products'][$j]['variants'][$k]['variant'][$l]['image_url']) ? $images[$i]['products'][$j]['variants'][$k]['variant'][$l]['image_url'] : null;
										if($_FILES['image_variant']['size'][$i][$j][$k][$l]>0) {
											$mime_type = getimagesize($_FILES['image_variant']['tmp_name'][$i][$j][$k][$l]);
											$img_ext = 'jpg';
						...
						// Upload additional images (image_1 to image_4) or preserve existing
						$additional_images = [
    ...
    private function update_link() {
        ...
        if($_POST['url'] == $link->url) {
			...
			$rs_lk = Database::$database->query("SELECT a.`url` as l_url, b.`url` as w_url FROM `links` a LEFT JOIN `whitelabel` b ON a.`user_id` = b.`user_id` WHERE a.`url` = '{$url}' AND (a.`domain_id` = '{$domain_id}' AND b.`url` = '{$main_server_name}')");
			$lk_exists = $rs_lk&&$rs_lk->num_rows ? $rs_lk->num_rows : false;
			if($lk_exists)
				Response::json($this->language->create_biolink_modal->error_message->url_exists, 'error');
    ...
    private function delete() {
        ...
        if(empty($errors)) {
			if($link->type == 'biolink' && $link->subtype != 'base') {
				...
				} elseif($link->subtype == 'link') {
					if($images&&isset($images->images)) {
						foreach($images->images as $im) {
							if($images->image&&file_exists(UPLOADS_PATH . 'galleries/' . $link->biolink_id . '/' . $images->image)) {
								unlink(UPLOADS_PATH . 'galleries/' . $link->biolink_id . '/' . $images->image);
							}
						}
					}
				}
