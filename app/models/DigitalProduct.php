...
class DigitalProduct {
    ...
    public static function migrate() {
        ...
        @Database::$database->query("ALTER TABLE `" . self::$table . "` ADD COLUMN `image_path` VARCHAR(512) NULL AFTER `access_url`");
        @Database::$database->query("ALTER TABLE `" . self::$table . "` MODIFY COLUMN `currency` VARCHAR(8) NOT NULL DEFAULT 'IDR'");

        return $ok;
    }

    public static function find_by_slug($slug) {
        return Database::get(['product_id','user_id','name','slug','description','price_cents','currency','file_path','access_url','image_path'], self::$table, ['slug' => $slug]);
    }

    public static function find_by_id($product_id) {
        return Database::get(['product_id','user_id','name','slug','description','price_cents','currency','file_path','access_url','image_path'], self::$table, ['product_id' => $product_id]);
    }
