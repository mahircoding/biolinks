...
define( 'PUBLIC_SUFFIX_LIST', UPLOADS_PATH . 'public_suffix_list.dat' );
...
class ManageSLD {
    ...
    function __construct( $rulestore=false ) {
        $this->setStore( $rulestore );
    }
    
    function setStore( $rulestore=false ) {
        if ( $rulestore) {
    ...
    function parseToStore() {
        $this->parse_to_store = true;
    }
    
    function parseToRuleset() {
    ...
    public static function applyRules($domain,$rules) {
        ...
        foreach ( $rules as $rule ) {
            ...
            for( $i=0; $i<$pattern_len; $i++ ) {
                ...
                if ( $p == '*' ) {
                    continue;
                } else if ( $i>$domain_len || $domain_parts[$domain_len-$i-1] != $p ) {
                    $mismatch = true;
                    break;
                } else {
                    continue;
        ...
        $tld = ManageSLD::getTLD($domain);
        if ( $tld == 'example' || $sld == 'example.com' ) $flags |= PUBLIC_SUFFIX_EXAMPLE_FLAG;
...
// for more dynamic use shoudl use an SQL backend, memcache, or something like that
...
class RuleStoreSLD {
    ...
    function addRule( $rule ) {
        $this->tld_index[$rule['tld']][] = $rule;
    }
    
    function getRules( $tld ) {
        return $this->tld_index[$tld];
    }
